<nav class="navbar navbar-inverse">
  <div class="container-fluid">
    <div class="navbar-header">
      <a class="navbar-brand" href="index" style="font-family:forte ;color:#1A5880;">BookWorld</a> 
      <img src="resources/images/openbook.png" style="max-height:45px;max-width:300px;">
    </div>
    <ul class="nav navbar-nav">
      <li class="active"><a href="index">Home</a></li>
      <li class="dropdown"><a class="dropdown-toggle" data-toggle="dropdown" href="books">BOOKS <span class="caret"></span></a>
        <ul class="dropdown-menu">
      <li class="dropdown-header">FEATURED STORE</li>
      <li><a href="bookuser?name=Novel">Novels</a></li>
      <li><a href="bookuser?name=kids book">Kids book</a></li>
       <li><a href="bookuser?name=Short Stories">Short Stories</a></li>
        <li><a href="bookuser?name=Academic Book">Academic books</a></li>
        <li><a href="bookuser?name=Literature and Fiction">Literature and Fiction</a></li>
      <li class="divider"></li>
      <li class="dropdown-header">KIDS AND TEENS</li>
      <li><a href="bookuser">Best Sellers</a></li>
      <li><a href="bookuser">Indian Author</a></li><li>
      <li><a href="bookuser">Activity Books</a></li>
      <li><a href="bookuser">Fun and Games</a></li>
       <li class="divider"></li>
        <li><a href="students">Admindetails</a></li>
    </ul>
      <li><a href="aboutus">About Us</a></li>
    <li><a href="contactus">Contact Us</a></li>
      
    </ul>
    <ul class="nav navbar-nav navbar-right">
      <li><a href="register"><span class="glyphicon glyphicon-user"></span> Register</a></li>
      <li><a href="signin"><span class="glyphicon glyphicon-log-in"></span> Login</a></li>
    </ul>
  </div>
</nav>
<br>